...
export function AdminNav({ name }: { name: string }) {
  return (
    <aside className="overflow-hidden rounded-3xl bg-stone-900 p-4 text-white sm:p-6 lg:sticky lg:top-6">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-[0.2em] text-orange-300 sm:text-sm">Xin chào</p>
          <h2 className="mt-2 text-xl font-black sm:text-2xl">{name}</h2>
        </div>
        <Link href="/" className="rounded-full border border-white/15 px-3 py-2 text-xs font-bold text-orange-200 sm:hidden">Site</Link>
      </div>
      <nav className="mt-5 flex gap-3 overflow-x-auto pb-1 text-sm lg:block lg:space-y-3 lg:overflow-visible lg:pb-0">
      ...
      <form action={logoutAdmin} className="mt-5 sm:mt-6">
        <button className="w-full rounded-full bg-orange-600 px-4 py-3 font-bold">Đăng xuất</button>
      </form>
    </aside>
  );
}
