        ...
        <section className="space-y-6">
          ...
          <div className="rounded-3xl bg-white p-5 shadow-sm sm:p-6">
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div>
                <p className="text-sm font-bold uppercase tracking-[0.2em] text-orange-700">Chi tiết đơn hàng</p>
                <h1 className="mt-2 text-3xl font-black sm:text-4xl">#{order.code}</h1>
                <p className="mt-2 text-stone-500">Tạo lúc: {order.createdAt.toLocaleString("vi-VN")}</p>
              </div>
              <div className="flex flex-wrap gap-3">
                <form action={updateOrderStatus} className="grid gap-3 sm:flex">
                  <input type="hidden" name="id" value={order.id} />
                  <select name="status" defaultValue={order.status} className="rounded-xl border p-3">
                    {statuses.map((status) => <option key={status} value={status}>{status}</option>)}
                  </select>
                  <button className="rounded-full bg-stone-900 px-5 py-3 text-center font-bold text-white">Cập nhật</button>
                </form>
            ...
            <aside className="space-y-6">
              ...
              <div className="rounded-3xl bg-white p-5 shadow-sm sm:p-6">
                ...
                <div className="mt-4 space-y-2 text-sm">
                  ...
                  <div className="flex justify-between border-t pt-3 text-lg font-black text-orange-700"><span>Tổng</span><span>{money(order.total)}</span></div>
                </div>
              </div>
            </aside>
          </div>
